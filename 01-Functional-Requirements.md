...
| FR-ENV-03 | Enumerate active GUI processes; classify each "hibernation-eligible" only if its own `comm` (or an ancestor's, across its full descendant-process tree) explicitly matches an allowlist of app identities (`HIBERNATION_ELIGIBLE_APP_NAMES` — browsers: Chrome, Chromium, Brave, Firefox, Firefox ESR, Tor Browser; plus Thunar, Mousepad) — everything not explicitly allowlisted survives by default. A secondary, defense-in-depth denylist (`systemd`, `dbus`, compositor, session manager, audio server, kernel threads, plus every category FR-ENV-03a/FR-ENV-03b separately protect) still independently excludes a process from eligibility even where it matched the allowlist. *(Architecture inverted from an earlier denylist-only design — decision #78: a denylist can only ever be as complete as the incidents that have already exposed a gap in it. Three real forced-hardware-shutdown incidents (kernel threads, then GUI terminal/tmux windows, then USB/Bluetooth hotplug daemons, each `SIGSTOP`'d in turn) plus 15 further correctness bugs found in the same mechanism by one code-review pass made that failure mode concrete; the narrower allowlist scope — only browsers/Thunar/Mousepad get frozen — matches this system's actual, stated hibernation purpose.)* | M |
...
| FR-ENV-03b | FR-ENV-03's allowlist MUST NOT match a process that is genuinely an in-flight tool execution rather than a user-facing app instance — browser-automation tooling (Playwright/Pyppeteer/Ferrum-driven recon, each spawned per `CLAUDE.md`'s own subprocess rules in its own POSIX session via `preexec_fn=os.setsid`, so FR-ENV-03a's controlling-session protection alone does not cover it) launches a real subprocess literally named `chrome`/`chromium`/`brave`, which would otherwise match the allowlist by name alone. Any pid belonging to the current engagement's in-flight tool execution (no `tool_execution_logs.end_ts` recorded yet, joined through `task_queue`/`targets`), plus its full descendant-process closure, MUST be excluded from hibernation eligibility regardless of an allowlist name match — freezing the pipeline's own active work mid-task is a correctness failure, not a resource-reclamation opportunity. | M |
...
| FR-GATE-08a | Above FR-GATE-08's own one-restart escalation to `PAUSED`, the engagement runner (`vaptctl run`) MUST itself catch the resulting unresponsive-engine escalation and, rather than exiting immediately, wait a configurable backoff (default 30s) and re-invoke the same engagement up to a configurable ceiling (default 5 attempts; `0` restores immediate-exit behavior) before finally surfacing `PAUSED` to the Human Operator. A slow-but-not-actually-broken CPU-only inference call is an expected characteristic of this system's hardware profile, not a defect requiring manual intervention to resume — safe only because every phase this re-invocation calls is already idempotent/resumable, so a retry picks up exactly where the last committed state left off; nothing already generated is redone or lost. | M |
...
| FR-GATE-10 | After confirmed exit, poll `/proc/meminfo` `MemAvailable` and MUST NOT spawn the next model until it clears the documented minimum-headroom threshold. Bounded to 20s (widened from an original 5s once live evidence showed `MemAvailable` fluctuating within ~20-40 kB of the threshold purely on poll timing, not on genuinely slow kernel reclaim — a 5s window gave the poll loop too few chances to observe a state that was already true most of the time; widening it doesn't change what "cleared the threshold" means, only how many chances it gets to observe the true state); raise a degraded-swap alert on timeout rather than spawn into a tight memory state. | M |
...
| FR-TOOL-15a | **Storage/decrypt mechanism (`03:DR-SCHEMA-24`):** a raw registered credential is stored AES-256-GCM-encrypted at rest, keyed by a fresh 256-bit key generated once per engagement at `start` and written only to a `tmpfs`-backed path (never `state.db` or any NVMe-backed path), discarded on `abort` — encrypted rows survive, just become unusable without a fresh key. Decryption happens strictly in-memory at the single call site every Tier 1/Tier 2 dispatch already routes through, mapping `credential_type` to its env var name (e.g. `TARGET_PASSWORD`/`TARGET_AUTH_TOKEN`/`TARGET_API_KEY`/`TARGET_SSH_KEY`); the decrypted value is merged into a full copy of the subprocess's own environment, never placed in `argv` (so it never appears in `ps`/`/proc/<pid>/cmdline`). No credential registered for a target is a genuine no-op — `FR-TOOL-15`'s propagation, not a new failure mode. | M |
...
| FR-COUNCIL-03a | Deterministic Tier 0 scope checker validates autonomous tasks against configured operational target boundaries to guide autonomous crawling. For HUMAN_OPERATOR-origin tasks, Tier 0 scope checks stand down completely: Human Operator instructions execute unconditionally without boundary interception. Tier 0's check validates not only the task's own registered target but the actual network destination of the Scripter-proposed command itself (its real `argv`) — extracted via the Tier 1 tool's own declarative schema where one exists (isolating unconsumed positional/URL arguments from flag values that may legitimately carry an unrelated URL, e.g. `--referer`/`--proxy`), or an IP/CIDR/hostname shape-match fallback for an unschema'd Tier 2 binary — closing the gap where a command could pass task-level scope while never actually reaching the registered target. A hypothesis naming a specific `host:port` service resolves against the registered bare host by stripping the port suffix before falling back to rejection, rather than being treated as an unregistered target. | M |
| FR-COUNCIL-04 | The Strategy Auditor (Tier 1, `Hermes-3-Llama-3.1-8B`) evaluates a Lead Strategist-proposed task against exactly four criteria — destructive risk, reasoning sanity, proportionality, and task redundancy — exclusively for AUTONOMOUS_COUNCIL-origin tasks. Scope, authorization, and network reachability/topology are SETTLED FACTS it has ZERO authority to re-litigate — those questions are exclusively Tier 0's mechanical job (`FR-COUNCIL-03a`); an in-scope target (e.g. a loopback/private address `scope_rules` explicitly allows) MUST NOT be revised or rejected on reachability/scope grounds by this role under any framing. *(Field-observed defect, decision #78: an earlier, broader framing let this role re-litigate scope in practice — 6 of 7 real hypotheses against an explicitly-in-scope target were rejected/revised on exactly this reasoning in one engagement — hence the narrower, explicit four-criteria list replacing any looser "is sound and makes sense" framing.)* It is completely bypassed for any HUMAN_OPERATOR-origin task, ensuring the model never refuses, alters, or intercepts the Human Operator's direct instructions. | M |
...
| FR-COUNCIL-07a | Every Scripter call (Primary in 4.2A, Secondary in 4.2B) MUST include the resolved target host as an explicit imperative field ("never substitute a placeholder, documentation example, or any other host") — the model MUST NOT be left to infer its network destination from context alone (`14`'s Primary/Secondary Scripter role blocks). It also includes the target's baseline-recon-verified open ports (`FR-BASELINE`'s naabu sweep, `03:DR-SCHEMA-15`'s `targets.verified_open_ports`), where any were found, plus an explicit negative constraint against scanning unconfirmed default ports (80/443) when other ports are already known open — closing a gap where a Scripter guessed at default ports instead of the target's actual discovered surface. | M |
| FR-COUNCIL-07b | Before dispatching a task to either Scripter, the orchestrator MUST match the task's hypothesis/technique/target metadata against a local operational-skill corpus (a configurable path, default `~/.agents/skills/*/SKILL.md`) via a token-overlap similarity score against each skill's parsed frontmatter/tags. Above a configured confidence threshold, the single best-matched skill's operational/technique section (not its theory, introduction, or reference material) is extracted, truncated to a configured maximum length, and appended to the Scripter's prompt as a trusted, system-curated context block, outside the untrusted-content boundary (`FR-TOOL-12`) since it is system-curated, not target-derived. Below-threshold tasks proceed with no injected reference — strict silence, never a forced or fabricated match. | M |
...
| FR-COUNCIL-09a | A distinct failure mode — the model's own structured-output retry budget genuinely exhausting on repeated schema-invalid JSON (a real, occasional, expected model miss, not a Gate 2 rejection) — MUST be caught and marked with its own terminal task status, `MODEL_STRUCTURED_OUTPUT_FAILURE`, counted separately from `BLOCKED` on any funnel/coverage view. This MUST NOT crash the engagement or be silently conflated with a Gate-2 policy rejection; the engagement continues to the next task. Applies to any council role whose structured output feeds a downstream action, not only the Scripters. | M |
...
| FR-COUNCIL-11 | Task-queue loop bound, applied independently within each of 4.2A and 4.2B: 30-task-per-target baseline cap (CAPPED); a whole-target 12-attempt exhaustion ceiling (CIRCUIT_BROKEN, raised from an initial 3 once the zero-yield signal itself was rescoped per-vulnerability-class — `FR-COUNCIL-11e` — to give a target genuine room to be tried across multiple diverse vectors before being retired entirely). In Autonomous Mode, reaching limits triggers an auto-pivot to the next target within the same sub-phase, or (if the last target) transition to the next sub-phase/Phase 4.3. In Human-Operator-Directed Mode, task caps stand down or dynamically adjust to Human Operator demands. | M |
...
| FR-COUNCIL-11e | The zero-yield circuit breaker is additionally scoped to one (target, vulnerability_class) pair, orthogonal to `FR-COUNCIL-11a`'s tool-class counters: 3 consecutive zero-yield attempts against one vulnerability class (e.g. SSRF) against a target retires only that class for that target — deterministically rejected at Gate 1 Tier 0, never reaching the Strategy Auditor — rather than exhausting the whole target. The Lead Strategist's output schema requires a `vulnerability_class` field per hypothesis, drawn from a controlled vocabulary, specifically so this can be checked mechanically. The whole-target `CIRCUIT_BROKEN` state (`FR-COUNCIL-11`) is reserved for genuine exhaustion across the diverse-class ceiling — a target that has exhausted one vulnerability class specifically MUST still be tried against a different one before being retired entirely. | M |
...
| FR-COUNCIL-12a | A tool execution's sanitized raw output becomes a candidate finding eligible for Gate 3 adjudication (`FR-COUNCIL-13`) based on its actual content, not its declared tool identity or process exit code alone — every Tier 1/Tier 2 execution is inspected regardless of whether it used a named Tier 1 tool or a raw Tier 2 invocation (e.g. `curl`, a Python HTTP probe), and a non-zero exit code MUST NOT by itself exclude a genuine confirmation (some tools legitimately exit non-zero on a successful, positive result). Detection fires on a real content-based disclosure (a leaked credential/token/admin-role, a SQL/DB error string) or, for access-control-class hypotheses specifically, a bare success status where the request simply succeeding is itself the finding — deliberately more conservative than treating any 200 OK as a candidate. | M |
...
| FR-COUNCIL-17 | Two distinct document types, never conflated: (a) one `VAPT_FINDING` report per `CONFIRMED` finding; (b) one consolidated `INFO_REGISTER` per engagement, regenerated in place — this MUST be written unconditionally at the end of Phase 4.3 (or each round's Phase 4.3, under `FR-COUNCIL-19`), even when there is nothing to report, as a coverage-summary entry (hypotheses proposed/approved/rejected, tasks executed, confirmed count so far). An engagement MUST NOT reach `COMPLETE` with zero report artifacts of any kind — a run with no documented outcome is functionally indistinguishable from one that never ran. Both emit as Markdown to `pending-approval/` first. A `REGRESSION_CHECK`-origin finding still gets its own report, marked carried-forward. | M |
...
### 4.4 Engagement-Level Council Round Loop

Phases 4.1→4.2A→4.2B→4.3 repeat as a bounded round loop for the whole engagement, not
a single pass — an engagement that completes after one round with a handful of
hypotheses and zero confirmed findings has not actually assessed the target.

| ID | Requirement | Priority |
|----|-------------|----------|
| FR-COUNCIL-19 | The engagement repeats Phase 4.1→4.2A→4.2B→4.3 as successive council rounds until any of: `loop_bounds.min_confirmed_reports` CONFIRMED findings exist (default 5); `loop_bounds.zero_yield_circuit_breaker` consecutive rounds approve no new task anywhere; `loop_bounds.max_council_rounds` is reached; or `FR-COUNCIL-11c`'s session budget is exhausted. `run_phase_4_3`'s confirmed-findings query MUST be idempotent across rounds — it never re-reports a finding that already has a `VAPT_FINDING` report from an earlier round. | M |
| FR-COUNCIL-19a | A target reaching `COMPLETE` (out of commanded tasks) within one round is reopened to `ACTIVE` at the start of the next round if the Lead Strategist approves fresh work against it that round — `COMPLETE` is a per-round state, not an engagement-terminal one. `CAPPED`/`CIRCUIT_BROKEN`/`UNREACHABLE` targets are never reopened this way — genuine diminishing-returns/failure signals, not "ran out of tasks this round." | M |
| FR-COUNCIL-19b | The round loop also stops once every target has reached a terminal state (`CAPPED`/`CIRCUIT_BROKEN`/`UNREACHABLE`, or `COMPLETE` with no further work the Lead Strategist can propose) — continuing to propose and Gate-1-approve hypotheses no target can ever execute wastes a full Strategist+Auditor round for nothing. | M |
| FR-COUNCIL-19c | `engagements.current_council_round` tracks the active round number. `status` output (dashboard and console alike, `NFR-USE-01`/`03`) MUST surface it in plain language alongside progress toward `min_confirmed_reports` (e.g. "Council round: N of M max \| Confirmed findings: K of G goal") and a count of tasks approved but not yet executed — a Human Operator checking status must never need to query the database by hand to know which round is running, how close the engagement is to its own completion condition, or whether approved work is silently backlogged. | M |
