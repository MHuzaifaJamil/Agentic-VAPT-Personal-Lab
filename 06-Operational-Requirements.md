...
| OPS-LIFECYCLE-03 | Shutdown via normal completion (engagement reaches `COMPLETE`) and shutdown via `abort` (SEC-KILL) MUST both guarantee Phase 5 hibernation-exit runs — an aborted engagement MUST still restore the operator's suspended applications, not leave them frozen. |
