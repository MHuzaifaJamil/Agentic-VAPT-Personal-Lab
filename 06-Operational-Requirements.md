...
| OPS-LIFECYCLE-05 | For the full duration of active orchestration (both a tmux-relaunched auto-start and a direct/manual invocation, uniformly), the system MUST hold an OS-level inhibition against system suspend/sleep/idle — the host MUST NOT suspend itself mid-engagement. The inhibition MUST release automatically on every exit path (normal completion, `PAUSED` escalation, an unhandled exception) and MUST NOT block or fail the engagement on a non-systemd host lacking the inhibition primitive — degrade to holding no lock, log it, and continue. *(Field-observed defect: an idle-timeout-triggered host suspend was caught occurring mid-engagement, survived only because the monotonic clock used for timeout budgets happens to stop advancing during suspend — not a guarantee this system should rely on going forward.)* |
