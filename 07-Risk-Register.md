...
rate limits, resource margins, and circuit breakers. Human-Operator-directed risks are accepted
...
| RISK-CROSSMACHINE | This doc set + report-formatting rules were authored on a different machine than the target. | H | M | Medium | None — flagged as a cross-machine transfer assumption requiring Human Operator action. | **Open — Human Operator action required** |
...
| RISK-OPERATORWASTE | The Primary Scripter can't see opt-in flag state, wastes budget on predictable refusals. | M | L | Low-Med | Flag state surfaced directly in the Primary Scripter's own per-task context. | Mitigated |
...
| RISK-NORATELIMIT | Nothing limited Primary Scripter spawn rate — DoS-adjacent risk for a 12h unattended run. | M | M-H | Med-High | Per-target spawn-rate limiting. | Mitigated |
| RISK-CONTEXTGROWTH | The resident Primary Scripter has no context-window management strategy against its 16k ceiling over a long per-target loop. | M | M | Medium | None — genuinely unresolved. | **Open** |
| RISK-CHECKPOINTBYPASS | Sensitive action classes run unattended without operational visibility. | L | M | Low | Autonomous proposals log structured checkpoint audit events for Human Operator visibility; direct Human Operator commands execute immediately (`approved_via = 'HUMAN_OPERATOR_DIRECTIVE'`) with full audit logging. | Mitigated |
...
## Risks Governed by Human-Operator-Directed Design

Under the Dual-Mode Architecture, operational flexibility and manual intervention take
precedence during Human-Operator-directed execution. **RISK-TIER2RESIDUAL** (execution of unlisted
Kali binaries upon explicit Human Operator command) and **RISK-UNCENSOREDGATE** (elimination of model
