...
def _pick_model(preferred: str = None) -> str | None:
    ...
    if preferred:
        ...
        if matches:
            return matches[0]
        # An explicitly requested model that isn't installed should not
        # silently fall back to something else — that hides a user typo
        # or a model that was never pulled.
        return None
...
"""
    ...
    def run_command(self, cmd: str, timeout: int = 120, cwd: str = None,
        ...
        its own. Only pass require_confirmation=False for commands that
        were never derived from LLM output or target-controlled content.
        """
        import subprocess as _sp
...
4. If not: output the NEXT command in a ```bash ... ``` block to dig deeper, or `EXPLOIT_DONE` if exhausted."""})
    ...
    def post_recon_hook(self, recon_dir: str, findings_dir: str = "") -> str:
        ...
        analysis_snip = analysis[:3000]

        prompt = f"""Based on recon of {target}, generate a targeted scan plan.

## Recon Analysis
{delimit_untrusted("recon analysis", analysis_snip) if analysis_snip else "(none)"}

## Live hosts (httpx)
{delimit_untrusted("live hosts (httpx)", httpx_sample) if httpx_sample else "(none)"}

## Priority hosts
{delimit_untrusted("priority hosts", priority_sample) if priority_sample else "(none)"}
