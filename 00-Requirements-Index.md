...
| 07 | [`07-Risk-Register.md`] | **(Informational)** Technical, resource, and operational trade-offs; categorizes mitigated controls versus accepted Human-Operator-directed design decisions. |
...
| 09 | [`09-Acceptance-Criteria-and-Test-Plan.md`] | Test suites (`TP-*`) and verification pass criteria validating autonomous containment alongside unconditional Human-Operator-directed execution. |
...
| 23 | [`23-Interactive-TUI-Console-and-Intervention-Pipeline-Specification.md`] | Interactive Human Operator console (`Textual`): append-only live audit journal, prefix routing (`@script`), and immediate Human Operator command dispatch. |
...
| Interactive TUI Console & Human Operator Intervention | `23` | `01`, `05`, `13` |
...
* **Human-Operator-Directed Mode:** Direct Human Operator commands, console dispatches, or manual scripts execute unconditionally with zero automated refusal, safety suppression, or technical gate interception.
...
4. **Local-Only Data Residency:** All target findings, credentials, and evidence artifacts remain strictly local to the Kali host. No unencrypted egress or external SaaS telemetry occurs without intentional Human Operator configuration.
