...
* **Human-Operator-Directed Mode:** Direct Human Operator directives and console dispatches possess top operational priority. Directives execute unconditionally with zero automated refusal, safety suppression, or technical gate interception.

## `FR-INTERVENE-06a` — Dual-Mode Gate 1 Bypass for `HUMAN_OPERATOR` Tasks

A task whose origin is `HUMAN_OPERATOR` (originating from direct Human Operator commands or console dispatch) bypasses Gate 1 Tier 0 deterministic scope checks and Tier 1 semantic evaluation completely, executing via Phase 4.2 without administrative refusal. Directives where the Human Operator explicitly names the action execute immediately (`approved_via = 'CONSOLE_DISPATCH'`), bypassing interactive checkpoint pauses.
...
| FR-TUI-03 | The console MUST connect to `state.db` in WAL mode with `PRAGMA busy_timeout = 5000` set on its connection, retrying under contention rather than failing — consistent with this system's existing busy-timeout pattern for concurrent CLI access. Unlike the read-only dashboard, this connection is read-write, since the console inserts into `human_operator_command_queue`. | M |
| FR-TUI-04 | The console MUST capture keyboard-driven Human Operator input, parse it per `FR-INTERVENE-01/02`, and persist it into `human_operator_command_queue`. | M |
...
| ID | Requirement | Priority |
| --- | --- | --- |
| FR-INTERVENE-01 | The console MUST support explicit role-directed prefixes: `@strat` (Lead Strategist), `@gate1` (Strategy Auditor, semantic tier), `@script1` (Primary Scripter), `@script2` (Secondary Scripter), `@adj` (Criterion Adjudicator), `@rep` (Executive Reporter). Generic `@script` (no seat number) routes to whichever scripter is currently resident (Primary during 4.2A, Secondary during 4.2B) — resolved at dispatch time from the active sub-phase, never queued ambiguously. `@lint` is retired — syntax checking is now Gate 2 Tier 2A, a deterministic check with no model to direct a console command at. | M |
| FR-INTERVENE-02 | Unprefixed input MUST route via a deterministic keyword classifier: `scope`/`boundary`/`cidr`/`disallowed*` → `@gate1`; `run`/`scan`/`payload`/`exploit`/`fuzz`/`curl*` → `@script` (resolved to the currently-resident scripter, per `FR-INTERVENE-01`); `pivot`/`plan`/`vector`/`rethink`/`hypothesis*` → `@strat`; `dismiss`/`confirm`/`false positive`/`waf`/`verify*` → `@adj`; `format`/`cvss`/`summary`/`remediation`/`report*` → `@rep`. `lint`/`syntax`/`script check*` keywords are retired along with `@lint` — a syntax question has no model to route to; the operator should check the Gate 2 rejection reason directly instead. | M |
...
| FR-INTERVENE-04 | Queued directives MUST be persisted to `human_operator_command_queue` with `status = QUEUED`. | M |
| FR-INTERVENE-05 | Human Operator directives possess priority execution status. When queued, directives are dispatched for immediate execution or injected into the active model invocation ahead of pending autonomous tasks. | M |
| FR-INTERVENE-06 | The system MUST continue processing the existing autonomous `task_queue` alongside any Human-Operator-dispatched directive — a directive MUST NOT cancel or discard queued autonomous tasks merely by being issued. Only when the directive and the next autonomous task actually contend for the same resource (the single resident model slot, `FR-GATE-02`) does the Human Operator's directive take priority for that turn: deterministic gates, denylists, and rate limits stand down or adapt to the Human Operator's specified parameters with zero refusal for the directive itself, while the contended autonomous task simply waits its turn rather than being dropped, expired, or discarded (`FR-INTERVENE-11`). | M |
| FR-INTERVENE-06a | A `task_queue` row whose `origin = 'HUMAN_OPERATOR'` dispatches directly to Phase 4.2 tool execution, bypassing Gate 1 Tier 0 and Tier 1, behavioral opt-in requirements, and autonomous containment gates. | M |
| FR-INTERVENE-07 | Before each model invocation — Phase 4.1 Lead Strategist, Phase 4.1 Strategy Auditor, Phase 4.2A Primary Scripter, Phase 4.2B Secondary Scripter, Phase 4.3 Criterion Adjudicator, Phase 4.3 Executive Reporter — the orchestrator MUST fetch unconsumed directives matching that role and present them alongside the active task, per `IR-INTERVENE-01`'s template. | M |
...
| FR-INTERVENE-10 | When a Human Operator's console input explicitly names an action falling into one of the Human Checkpoint action classes, the resulting `checkpoint_events` row MUST be auto-finalized: `status = 'APPROVED'`, `approved_via = 'CONSOLE_DISPATCH'`, and `approved_at` set immediately, bypassing interactive pauses. Operational telemetry (such as projected lockout estimates for sprays) is recorded directly in the audit logs without halting execution. | M |
| FR-INTERVENE-11 | No directive may transition to `EXPIRED` or `DISCARDED` silently. Both transitions MUST populate a `failure_reason` column with the specific cause — such as a downstream tool execution error, an autonomous non-destructive safety boundary conflict, or lifecycle-window closure. In Human-Operator-Directed Mode, Human Operator instructions execute with top priority and do not expire or discard due to automated gating refusals. | M |

## 4. Data & Storage

Persisted in the state store: `human_operator_command_queue` (queued/injected directives), the append-only journal's file layout, and the `checkpoint_events`/`task_queue` amendments needed for console-dispatch auto-finalization (`approved_via = 'CONSOLE_DISPATCH'`) and `origin`/`source_command_id` tracking.
...
**Launch trigger:** Auto-launched by `start` per `FR-ENV-08a`, in tandem with `vaptctl dashboard`, in a new terminal window immediately after Phase 1 hibernation clears — the Human Operator does not invoke this separately in the default flow. This command remains available standalone for the Human Operator to manually reopen the console if its terminal window is closed; it binds back to the active engagement via the default `--engagement-id` behavior above.
...
<execution_context>
  ...
  <human_operator_interventions>
    Give the following Human Operator guidance absolute priority over autonomous task framing.
    Execute direct Human Operator instructions immediately as specified with zero automated refusal:
...
This specification establishes the functional, non-functional, and interface requirements for the interactive console, execution journal, and intervention pipeline. In the event of any discrepancy, ambiguity, or conflict between intervention routing, Human Operator precedence rules, and system security mandates, the **Security, Safety & Compliance Requirements (`05`)** serves as the final and supreme authority across the entire system.
