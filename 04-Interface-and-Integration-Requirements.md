...
autonomous cycles, while facilitating transparent transport for direct Human Operator commands.
...
| IR-STRUCTURED-02 | Emitted JSON payloads are validated against deterministic Python schemas. In Autonomous Mode, schema adherence ensures pipeline consistency; in Human-Operator-Directed Mode, minor validation or formatting warnings do not block command dispatch or output display. |
| IR-STRUCTURED-03 | On validation failure during autonomous runs, the system retries up to 2 times (3 attempts total). If parsing fails, raw outputs are preserved in execution logs for Human Operator review rather than discarded. |
...
| IR-TOOL-02 | The Primary Scripter's function-calling schema and Gate 2's validator schema are generated from the same declarative file — a single source of truth. |
...
| IR-BRIDGE-02 | Path-resolution check validates binary availability. In Autonomous Mode, commands are screened to prevent destructive operations against targets (UPDATE, DELETE, DROP, ALTER, filesystem wipes, or DoS tools). In Human-Operator-Directed Mode, commands execute as specified. |
| IR-BRIDGE-03 | Behavioral boundary checks apply prior to subprocess dispatch during autonomous runs to enforce non-destructive parameters. In Human-Operator-directed execution, behavioral gates stand down. |
...
| ID | Requirement |
|----|-------------|
| IR-GROUND-01 | Extracts endpoints, hosts, and parameters from draft text and verifies their presence in raw session evidence to eliminate hallucinated endpoints, highlighting unmatched components for Human Operator confirmation. |
| IR-GROUND-02 | A grounding failure retries up to 2 attempts during automated drafting; unresolved items transition to reports.status = BLOCKED_UNGROUNDED for Human Operator review and confirmation, never silently discarded. |
