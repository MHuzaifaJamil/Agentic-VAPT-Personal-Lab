...
| SEC-DATA-05 | A registered target credential's raw value (`01:FR-TOOL-15a`, `03:DR-SCHEMA-24`) is the one exception to `SEC-DATA-04`'s "never the raw value" rule that still satisfies `SEC-DATA-01`'s local-only guarantee — it must actually be recoverable to be usable at all, unlike a one-way correlation hash. It is stored AES-256-GCM-encrypted at rest, never in plaintext, keyed by a fresh per-engagement key that itself lives only on `tmpfs` (never `state.db` or any NVMe-backed path) and is discarded at `abort` — an encrypted row surviving with no usable key is the intended "expire cleanly" behavior, not a data-loss defect. |
