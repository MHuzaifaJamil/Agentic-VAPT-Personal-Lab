...
|---|---|---|---|
| `01-Functional-Requirements.md` | modified | #77 | New `FR-TOOL-20` (Phase 4.2 AI-gated exploitation & specialist tools: `dalfox`/`xsstrike`/`ghauri`/`fuxploider` new; `hashcat`/`cupp`/`trevorspray`/`kerbrute`/`interactsh-client`/`mobsf`/`objection` formally registered, several already required by name in `19`'s prose); new `FR-DISCOVER` cluster (`01`–`04`: consult `KALI-TOOL-CATALOG.md` for undefined domains, `shutil.which` presence check, no autonomous install without opt-in, `ACTIVE_WIRELESS_DISRUPTION` checkpoint class for active wireless/Bluetooth actions); `FR-CHECKPOINT-01`'s fixed class list extended five→six. |
| `19-Extended-Capability-Domains.md` | modified | #77 | New `FR-MOBILE-08`: `objection` formally registered (already named in `FR-MOBILE-03`/`05`), `mobsf` added as new capability. Dependency Summary updated with `hashcat`/`cupp`/`trevorspray`/`kerbrute`/`dalfox`/`xsstrike`/`ghauri`/`fuxploider`/`mobsf`. |
| `09-Acceptance-Criteria-and-Test-Plan.md` | modified | #77 | New `TP-DISCOVER` cluster; `TP-TIER1` gained 3 rows (exploitation-tool registration, already-required-tool formal registration, mobile-tool registration); `TP-CHECKPOINT` gained a row for the six-class fixed list. |
| `18-Requirement-to-Test-Traceability-Matrix.md` | modified | #77 | Doc `01` 119→124 (83 covered); corpus baseline 361→366 (190 covered). |
| `10-Decision-Log-and-Open-Questions.md` | modified | #77 | Decision #77 entry added. |
| `STAGING-Pending-Discussions-and-Fixes.md` | modified | — | Rounds 6, 7, 8 moved from "Currently Pending Approval" into the archive, marked merged (decision #77), with the requested gap-check result recorded against Round 6. |
