...
progress, and circuit-breaker strikes, while surfacing Human-Operator-directed command activity and
...
| FR-DASHBOARD-04 | `SIGINT` (Ctrl+C) MUST be caught to restore normal terminal state (cursor visibility, exit the alternate screen buffer) before exiting — a dashboard crash or forced exit MUST NOT leave the Human Operator's terminal in a broken state. | M |
...
| FR-DASHBOARD-07 | The `RESIDENT`/`IDLE` display state (a model loaded in RAM but not currently generating) MUST only ever be shown for the **Primary Scripter** role — it is the only role that stays resident across multiple tasks in a per-target loop. Every other role (Lead Strategist, Strategy Auditor, Criterion Adjudicator, Executive Reporter) tears down immediately after each single invocation and MUST only ever display as `COLD` or `RUNNING`, never `RESIDENT`/`IDLE` — showing a non-Primary-Scripter role as `RESIDENT` reflects a display bug, not a valid state, and SHOULD be flagged as such if it occurs. | M |
...
| FR-DASHBOARD-09 | Predictive remaining-turn count (`N_exp`) per role, computed each refresh cycle: **Primary Scripter** (Phase 4.2A) / **Secondary Scripter** (Phase 4.2B) — each independently, `N_exp = T_pending + ceil(T_pending × max(0.10, R_retry))`, where `T_pending` is the count of `task_queue` rows with `status IN ('GATE1_APPROVED', 'PENDING')` for that sub-phase and `R_retry = GATE2_BLOCKED_count / EXECUTED_count` (a floor of 0.10 avoids an underestimate when few or zero retries have occurred yet); **Gate 1** — count of remaining unvetted Strategist hypotheses; **Gate 2 (deterministic, all three tiers)** — N/A, no model, see `FR-DASHBOARD-11`; **Gate 3** — `COUNT(verified_vulnerabilities WHERE status = 'CANDIDATE')`; **Reporter** — `1` if Phase < 5, `0` once Phase 5 is reached and reporting is complete. All `N_exp` values are estimates, and MUST be visually marked as such (e.g. an `[EST.]` tag), never presented as guaranteed counts. | M |
...
| FR-DASHBOARD-11 | Gate 2 (the deterministic three-tier command/argument/syntax/duplicate-vector validator, `01:FR-COUNCIL-08`) MUST be displayed as its own row with zero model dependence — no LLM is invoked at any of its three tiers. Gate 2's row MUST show a fixed `N/A (deterministic)` in place of every model-only field (state badge, tok/s, turn count) — it never has a "turn," a "speed," or a "resident" state. There is no separate linter-model row: syntax checking (formerly a distinct model invocation) is now Gate 2 Tier 2A, entirely deterministic. | M |
| FR-DASHBOARD-12 | **Cold-start fallback priors** (used only until a role records its own first real turn, per `FR-DASHBOARD-10`): Lead Strategist (`DeepSeek-R1-0528-Qwen3-8B`) ≈ 10.7 tok/s (~45s/turn); Primary Scripter (`Qwen2.5-Coder-7B-Instruct`) ≈ 28.5 tok/s (~15s/turn); Strategy Auditor (`Hermes-3-Llama-3.1-8B`) ≈ 11.0 tok/s (~8s/turn); Secondary Scripter (`DeepSeek-Coder-6.7B-Instruct`) ≈ 24.0 tok/s (~18s/turn); Criterion Adjudicator (`Mistral-7B-Instruct-v0.3`) ≈ 12.1 tok/s (~12s/turn); Executive Reporter (`Ministral-8B-Instruct-2410`) ≈ 11.0 tok/s (~180s for a full multi-pass synthesis). These are placeholder estimates for display purposes only, sourced from no benchmark run on this system's actual confirmed `Q8_0` roster — they MUST be labeled `[ESTIMATING]` in the UI and MUST NOT be treated as validated performance figures; a real pre-flight benchmark measures actual throughput separately and independently. | S |
...
| `○ RESIDENT / IDLE` (bold cyan) | **Primary Scripter only** | Loaded, not currently generating, between tasks in its per-target loop. Never valid for any other role. |
| `◌ COLD / EVICTED` (dim blue) | Any role | No resident weights; the normal state for every non-Primary-Scripter role between invocations, and for all roles before an engagement starts. |
...
The Gate 2 row shows `N/A (deterministic)` fields per `FR-DASHBOARD-11` (no separate
linter-model row exists), and the model matrix never shows two simultaneously
non-`COLD` rows in a genuine live snapshot per `FR-DASHBOARD-08` — including across the
Primary Scripter (4.2A) → Secondary Scripter (4.2B) handoff, which is a full unload/load
cycle, not an overlap.
...
- **Launch trigger:** Auto-launched by `start` per `FR-ENV-08a` in a new terminal window, immediately after Phase 1 hibernation clears — the Human Operator does not invoke this separately in the default flow. The standalone `vaptctl dashboard` command remains available for the Human Operator to manually reopen this view if its terminal window is closed.
