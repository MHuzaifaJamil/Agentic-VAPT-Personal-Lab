...
## TP-TUI — Interactive Console & Intervention Pipeline (FR-TUI-01..06, FR-STREAM-01..05, FR-INTERVENE-01..11 + 06a, `23-Interactive-TUI-Console-and-Intervention-Pipeline-Specification.md`)
...
| Console detects and stops on engagement-state change | Test | Bind a console to an `IN_PROGRESS` engagement, then `abort` it from elsewhere; confirm the console detects the transition and stops accepting new interventions rather than silently continuing to queue them (FR-TUI-06). |
| Offline Linter has full parity with the other 5 roles | Test | Queue a `@lint` directive ahead of a custom multi-line script's between-phase syntax check; confirm it's fetched and injected per `FR-INTERVENE-07`, the same as any other role. |
| Operator precedence affects reasoning, never bypasses gates | Test | Queue an `@op` directive that, if followed literally, would resolve to an out-of-scope or denylisted command; confirm the Operator's resulting proposal is still refused by Gate 1/Tier 2 exactly as an equivalent autonomous proposal would be (FR-INTERVENE-06). |
| Manual-operator origin skips Gate 1's semantic tier, never its deterministic tier | Test | Dispatch an explicit, specific `@op` command that is in-scope (passes `FR-COUNCIL-03a`); confirm `Hermes-3` (`FR-COUNCIL-04`) is never invoked for that task, `origin='MANUAL_OPERATOR'` and `source_command_id` is set. Repeat with an explicit, specific command that is out of scope; confirm `FR-COUNCIL-03a` still rejects it, same as an autonomous task. Repeat with a vague `@op` nudge the model expands into a specific command on its own; confirm `origin='AUTONOMOUS_COUNCIL'`, `source_command_id` is `NULL`, and the full two-tier gate runs (FR-INTERVENE-06a, decision #63). |
| Explicit console dispatch of a checkpoint action auto-attests, without touching the flag gate | Test | With `--allow-live-credential-spray` set, dispatch `@op spray <hostname> now` explicitly via console; confirm a `checkpoint_events` row is created and immediately finalized (`status='APPROVED'`, `approved_via='CONSOLE_DISPATCH'`) with no `PAUSED_AWAITING_CHECKPOINT` pause, and the lockout-percentage rationale is still echoed to the console. Repeat with the flag unset; confirm `POLICY_REFUSED` with no `checkpoint_events` row at all (FR-INTERVENE-10, FR-CHECKPOINT-02 unaffected). |
| Model-derived (not operator-explicit) checkpoint actions still pause | Test | Queue a vague `@op` directive that the Operator model then independently escalates into a live-spray proposal on its own; confirm the full `FR-CHECKPOINT-03` pause still fires — the auto-attestation shortcut applies only to the operator's own explicit, specific text (FR-INTERVENE-10). |
| No silent expiration | Test | Queue a directive for `@strat` against a target whose Phase 4.1 has already concluded; confirm it transitions to `EXPIRED` with `failure_reason` populated with the specific lifecycle-closure explanation (FR-INTERVENE-11). |
| Journal/console content is unredacted, same as raw artifacts | Inspection | Trigger a tool run that discovers a secret-shaped string; confirm the journal's "Signal Retained" block shows it verbatim, matching `tool_execution_logs`/`artifacts_index` — confirm this is *not* routed through `FR-COUNCIL-18`'s redaction pipeline, which stays scoped to the Reporter/report-draft step only. |
...
## TP-DEDUP — Historical State Inheritance & Deduplication (FR-DEDUP-01..07, `24-Historical-State-Inheritance-and-Deduplication-Specification.md`)

| Test | Method | Pass Criteria |
|---|---|---|
| Strategist receives exclusion context, doesn't repeat prior paths | Test | Run two sequential engagements against the same `host_or_domain`; confirm the second engagement's Strategist prompt contains `<explored_attack_paths>` derived from the first engagement's `task_queue`-executed paths (FR-DEDUP-01). |
| Gate 2 blocks an exact-duplicate command via the existing retry pipeline | Test | Re-execute an identical tool invocation against the same target in a second engagement; confirm Gate 2 rejects with `DUPLICATE_COMMAND` via the existing `FR-COUNCIL-09` regenerate/`BLOCKED` flow, and `targets.task_count` is not incremented for the blocked attempt (FR-DEDUP-02). |
| Command-hash canonicalization doesn't produce false-positive duplicates | Test | Submit two commands with swapped flag/value pairings (e.g. `-p 80 -oN 443` vs `-p 443 -oN 80`); confirm they hash differently and neither is falsely blocked. |
| `RETEST` mode always surfaces a visible outcome, never a silent drop | Test | Seed a prior `CONFIRMED` finding; run `--assessment-mode retest` against the same target. If the repro still succeeds, confirm an individual `VAPT_FINDING` report is generated, explicitly marked carried-forward. If it no longer succeeds, confirm `status='REMEDIATED'` and the finding appears in the `INFO_REGISTER` as a verified fix (FR-DEDUP-06). |
| `HISTORICAL_REGRESSION` origin gets zero gate exceptions | Test | Seed a prior finding whose stored repro now falls outside a narrowed `scope_rules`; run `--assessment-mode retest`; confirm Gate 1's full two-tier check rejects it exactly as a fresh out-of-scope proposal would (FR-DEDUP-05). |
