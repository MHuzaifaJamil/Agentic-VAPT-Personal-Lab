...
| Swap growth tracked and abnormal growth flagged | Test | Run an engagement segment that forces repeated hibernate/thaw cycles; confirm cumulative bytes paged to `/dev/nvme0n1p8` and `/swapfile` are tracked per session and logged (NFR-RES-06, NFR-REL-05's logging obligation). *No numeric "abnormal growth" threshold is defined anywhere in `01`-`13`, so this can only confirm the tracking/logging mechanism exists and increments correctly — the separate NFR-REL-05 claim that repeated use "does not measurably shorten NVMe lifespan" is a hardware-endurance claim not verifiable by a short-duration test, and belongs with `TP-FEASIBILITY` (verifiable only via long-run deployment monitoring), not here.* |
