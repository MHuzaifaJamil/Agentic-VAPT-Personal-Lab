...
| NFR-RES-02 | The system MUST maintain a minimum safety margin of **1.5 GB** free RAM above whatever the largest currently-loading model requires, aborting the load and pausing the engagement if the margin cannot be met. **[PROPOSED]** |
...
| NFR-RES-04 | The system MUST monitor NVMe root partition usage and MUST warn the operator at 85% utilization and hard-block new artifact writes at 95% utilization of the 185 GB root volume. **[PROPOSED]** |
| NFR-RES-05 | The system MUST pin LLM inference threads to the 4 P-Cores (8 threads) and MUST NOT consume more than 6 of the 8 E-core-scheduled threads for concurrent tool subprocesses, to preserve responsiveness of the linter/orchestrator loop. **[PROPOSED, derived from base §1.1 core topology]** |
...
| NFR-PERF-02 | Model swap (unload N → load N+1) MUST complete, end-to-end, within a bounded time budget of **60 seconds** under normal conditions, else the phase transition is logged as degraded. **[PROPOSED]** |
...
| NFR-REL-04 | The operator abort/kill-switch (FR-CTRL-04) MUST bring all subprocess trees and the inference engine to a fully stopped state within **10 seconds** of invocation. **[PROPOSED]** |
...
| NFR-REL-06 | The system MUST NOT lose unsaved user application state as a side effect of hibernation; freezing (`SIGSTOP`) is permitted, but the system MUST NOT terminate or otherwise force-close user applications to reclaim memory. |
