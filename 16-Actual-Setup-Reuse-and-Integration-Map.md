...
All security policies, execution boundaries, and Human-Operator-override guarantees governing these tools derive authoritatively from the **Security, Safety & Compliance Requirements (`05`)**.
...
| TR-SCRIPT-06 | **Baseline Reconnaissance Pipeline:** The recon/content-discovery technique modeled in `full_hunt.sh` and `dork_runner.py` (passive recon → crawling → parameter extraction → pattern-based candidate categorization) is reimplemented natively — not a literal bash port — as a fixed, deterministic pipeline that runs automatically once per target, with zero AI model invocation, immediately after Phase 3 completes and before the Lead Strategist's first Phase 4.1 invocation. See `01:FR-BASELINE` for the full pipeline specification and phase-trigger mechanics. | M |
| TR-SCRIPT-07 | **Dual-Mode Execution Posture for Tools:** All integrated scripts MUST execute according to active operating mode: in **Autonomous Mode**, operations MUST be strictly non-destructive (discovery reads and benign verification writes only; data drops, schema modifications, and DoS are prohibited); in **Human-Operator-Directed Mode**, scripts execute as parameterized with zero automated refusal or safety gating. | M |
...
| TR-BRIDGE-03 | **Caido MCP Client Adapter:** If Caido integration is enabled via runtime configuration, the system MUST interface with the community-maintained `caido-mcp-server` (`c0tton-fluff/caido-mcp-server` — there is no official `@caido` npm package; corrected to match the actual reference client in `Actual-Setup/mcp/caido-mcp-client/`) using the standard Model Context Protocol client specification, authenticated via a Caido Personal Access Token (`CAIDO_PAT` env var, or the server's own cached-token OAuth login flow as an alternative). | S |
...
| TR-METHOD-03 | **Cluster Hunting & Sibling Mapping:** Upon confirming a vulnerability on an endpoint, the Primary Scripter MUST map sibling endpoints within the same module/controller and formulate targeted follow-on tasks to assess blast radius before concluding exploration of that vector. | M |
...
| ID | Requirement | Priority |
| --- | --- | --- |
| TR-EXCLUDE-01 | **Interactive Copilot Commands:** Slash commands (`commands/*.md`) designed for interactive cloud-copilot runtimes are excluded from direct implementation; Human Operator interaction is owned exclusively by the `vaptctl` CLI surface and interactive TUI console. | M |
