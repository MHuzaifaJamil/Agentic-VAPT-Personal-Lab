...
| TR-SCRIPT-06 | **Reconnaissance & Pre-Filtering Pipeline:** The content-discovery and filtering pipeline modeled in `full_hunt.sh` and `dork_runner.py` (passive recon → crawling → parameter extraction → pattern-based candidate categorization prior to vulnerability-specific tool dispatch) MUST be implemented as the standard task-generation pattern for the Lead Strategist and Primary Scripter roles. | M |
