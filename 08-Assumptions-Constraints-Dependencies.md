...
| AC-DEPENDENCY-20 | **(New — required by `22-VAPT-Monitoring-Dashboard-Specification.md`)** The monitoring dashboard needs `rich` (TUI rendering, `Live`/`Layout`/`Panel`), `plotext` (in-terminal line-graph rendering), and `psutil` (CPU/RAM/thermal host telemetry) — none of which were previously part of this system's confirmed Python dependency floor (Click, PyYAML, `cvss`, an OpenAI-compatible HTTP client). | Nobody's job yet — same "flagged, not resolved" status as every other dependency in this table. |
| AC-DEPENDENCY-21 | **(New — required by `23-Interactive-TUI-Console-and-Intervention-Pipeline-Specification.md`)** The interactive console needs `Textual` (the TUI framework the console's layout/rendering is built on) — a separate dependency from the dashboard's `rich` (Textual is itself built on `rich`, but is a distinct package with its own event-loop/widget model). | Nobody's job yet. |
